...
public class DeploymentEditor : EditorWindow {
	...
	public static void Deploy() {
		...
		modPath = modPath.Substring(0, modPath.LastIndexOf('/')) + Path.DirectorySeparatorChar + "build" + Path.DirectorySeparatorChar + ModConfig.ID;

		DirectoryCopy(modPath, EditorPrefs.GetString(DEPLOYKEY) + Path.DirectorySeparatorChar + ModConfig.ID, true);
	...
	private void OnGUI() {
		...
		EditorPrefs.SetBool("AUTODEPLOY", EditorGUILayout.Toggle("Auto-deploy", (EditorPrefs.HasKey("AUTODEPLOY") ? EditorPrefs.GetBool("AUTODEPLOY") : true)));
